...
function Discover({ events, user, org, isRegistrations, isMyEvent, onDeleted }) {
  ...
  const organizations =
    notNullEvents.length > 0
      ? [
          ...new Set(
            notNullEvents.map(
              event => event.eventOwner?.orgName || event.orgName || 'Admin-created',
            ),
          ),
        ]
      : []
  ...
  const spotsLeft = selectedEvent
    ? Math.max(0, (selectedEvent.maxAttendees ?? 0) - attendeesCount)
    : 0
  const alreadyRegistered =
    selectedEvent && user ? selectedEvent.eventAttendees?.some(a => a.id === user.id) : false
