            ...
            user.tickets.forEach(ticket => {
              ...
              if (ticketWeekLabel === weekLabel) {
                pointData.push({
                  ...
                  registeredDate: ticket.createdAt
                    ...
                    : 'N/A',
                  attendedDate: ticket.validatedAt
            ...
            user.tickets.forEach(ticket => {
              if (ticket.status === 'CHECKED_IN') {
                usersWhoAttended.push({
                  ...
                  email: user.email,
                  eventTitle: ticket.event?.title || 'N/A',
                  checkInDate: ticket.validatedAt
                    ? new Date(ticket.validatedAt).toLocaleDateString('en-US', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric',
    ...
    <div className="analytics-page">
                    ...
                    <tbody>
                      {registeredUsers.map((user, index) => (
                        <tr key={`${user.userId}-${index}`}>
                          ...
                          <td>
                            <span
                              className={`analytics-status-badge analytics-status-${user.ticketStatus.toLowerCase()}`}
                            >
                    ...
                    <tbody>
                      {chartPointData.map((data, index) => (
                        <tr key={`${data.email}-${index}`}>
                          ...
                          <td>
                            <span
                              className={`analytics-status-badge analytics-status-${data.attended.toLowerCase()}`}
                            >
                              {data.attended}
                            </span>
                            {data.attended === 'Yes' && (
      ...
      {showStatsModal && detailedStats && (
        <div className="analytics-modal-overlay" onClick={() => setShowStatsModal(false)}>
          <div
            className="analytics-modal-content analytics-stats-modal"
          ...
          >
            ...
            <div className="analytics-modal-body">
                ...
                <div className="analytics-stat-card">
                  ...
                  <div className="analytics-stat-info">
                    ...
                    <div className="analytics-stat-value">
                      {detailedStats.avgAttendancePerEvent}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="analytics-stats-summary">
                <h3>Summary</h3>
                <p>
                  Out of <strong>{detailedStats.totalTickets}</strong> tickets issued across{' '}
                  <strong>{detailedStats.totalEvents}</strong> events,
                  <strong className="analytics-text-success">
                    {' '}
                    {detailedStats.totalAttendance}
                  </strong>{' '}
                  attendees checked in (<strong>{detailedStats.attendanceRate}%</strong> attendance
                  rate) while
                  <strong className="analytics-text-danger"> {detailedStats.noShows}</strong> were
                  no-shows (<strong>{detailedStats.noShowRate}%</strong>).
                </p>
                <p>
                  On average, each event has <strong>{detailedStats.avgTicketsPerEvent}</strong>{' '}
                  registered participants with{' '}
                  <strong>{detailedStats.avgAttendancePerEvent}</strong> actual attendees.
                </p>
              </div>
            </div>
          </div>
