...
import {
  ...
  FaBuildingCircleCheck /*, FaBuildingCircleXmark*/,
} from 'react-icons/fa6'
import usePageTitle from '../../hooks/usePageTitle'

export default function UserModerations({
  organizations,
  setOrganizations,
  users,
  setUsers,
  ...
  const handleApproveOrganization = async (e, orgId) => {
    ...
    if (res.ok) {
      ...
      const updatedOrganizations = organizations.map(org =>
        org.id === orgId ? { ...org, isApproved: true } : org,
      )
  ...
  const handleUnapproveOrganization = async (e, orgId) => {
    ...
    if (res.ok) {
      //updates local copy
      const updatedOrganizations = organizations.map(org =>
        org.id === orgId ? { ...org, isApproved: false } : org,
      )
      setOrganizations(updatedOrganizations)
    }
  }
          ...
          {orgRequests.map(org => (
            <div key={org.id} className="moderation-card">
              ...
              <div className="card-actions">
                <button className="approve-btn" onClick={e => handleApproveOrganization(e, org.id)}>
                  Approve
                </button>
                <button
                  className="reject-btn"
                  onClick={e => handleDeleteOrganization(e, org.authId)}
                >
                  Reject
                </button>
              </div>
          ...
          {orgApproved.map(org => (
            <div key={org.id} className="moderation-card">
              ...
              <div className="card-actions">
                <button
                  className="reject-btn"
                  onClick={e => handleUnapproveOrganization(e, org.id)}
                >
                  Disable
                </button>
                <FaTrashAlt
              ...
              <div className="card-actions">
                {user.role === 'USER' ? (
                  <button
                    ...
                    onClick={e => handleChangeAdminStatus(e, user.id, 'ADMIN')}
                  >
                    Make Admin
                  </button>
                ) : (
                  <button
                    className="reject-btn"
                    onClick={e => handleChangeAdminStatus(e, user.id, 'USER')}
                  >
                    Remove Admin
                  </button>
                )}
                <FaTrashAlt
                  className="trash-icon"
                  onClick={e => handleDeleteUser(e, user.authId)}
                />
              </div>
            </div>
          ))}
