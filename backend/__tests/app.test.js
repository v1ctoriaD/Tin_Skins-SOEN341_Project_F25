...
describe('Express API routes (mocked)', () => {
  ...
  test('POST /api/signup user succeeds', async () => {
    const res = await request(app)
      .post('/api/signup')
      .send({
        formData: { email: 'user1@example.com', password: 'pass', firstName: 'A', lastName: 'B' },
        accountType: 'user',
      })
    expect(res.status).toBe(200)
  ...
  test('POST /api/signup organization succeeds', async () => {
    const res = await request(app)
      .post('/api/signup')
      .send({
        formData: { email: 'org1@example.com', password: 'pass', organizationName: 'OrgX' },
  ...
  test('POST /api/moderate/user ChangeAdminStatus succeeds', async () => {
    const res = await request(app)
      ...
      .send({ reqType: 'ChangeAdminStatus', userId: 1, role: 'admin' })
    expect(res.status).toBe(201)
  })
  ...
  test('POST /api/moderate/user ApproveOrganization succeeds', async () => {
    const res = await request(app)
      .post('/api/moderate/user')
      .send({ reqType: 'ApproveOrganization', orgId: 1 })
    expect(res.status).toBe(201)
  })
  ...
  test('POST /api/moderate/user DeleteUser succeeds', async () => {
    const res = await request(app)
      .post('/api/moderate/user')
      .send({ reqType: 'DeleteUser', authId: 'auth1' })
    expect(res.status).toBe(201)
  })
})
